 ...
 * Creates all 48 tables for the TOM system
 * 
 ...
 * 
 * Strategy:
 * 1. Create organizations (without created_by FK)
 * 2. Create users
 * 3. Add created_by FK to organizations
 * 4. Create projects (without sales_order_id FK)
 * 5. Create sales_orders (can reference projects)
 * 6. Add sales_order_id FK to projects
 * 7. Create all other tables in dependency order
 */

exports.up = async function(knex) {
  // Step 1: organizations (without created_by FK)
  await knex.raw(`CREATE TABLE organizations (
  ...
  await knex.raw(`CREATE INDEX idx_organizations_code ON organizations(code);
...
CREATE INDEX idx_users_deleted_at ON users(deleted_at) WHERE deleted_at IS NULL;`);
  ...
  // Step 3: Add created_by FK to organizations
  await knex.raw('ALTER TABLE organizations ADD CONSTRAINT fk_organizations_created_by FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL');

  // items
  await knex.raw(`CREATE TABLE items (
    id SERIAL PRIMARY KEY,
    code VARCHAR(50) UNIQUE NOT NULL,
    name VARCHAR(255) NOT NULL,
    description TEXT,
  ...
  await knex.raw(`CREATE INDEX idx_items_code ON items(code);
CREATE INDEX idx_items_category ON items(category);
CREATE INDEX idx_items_status ON items(status);`);

  // sales_enquiries
  await knex.raw(`CREATE TABLE sales_enquiries (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_sales_enquiries_enquiry_number ON sales_enquiries(enquiry_number);
...
CREATE INDEX idx_sales_enquiries_created_at ON sales_enquiries(created_at);`);
  ...
  // sales_quotations
  await knex.raw(`CREATE TABLE sales_quotations (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_sales_quotations_quotation_number ON sales_quotations(quotation_number);
...
CREATE INDEX idx_sales_quotations_status ON sales_quotations(status);`);
  ...
  await knex.raw(`CREATE TABLE sales_quotation_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_sales_quotation_items_quotation_id ON sales_quotation_items(quotation_id);
CREATE INDEX idx_sales_quotation_items_item_id ON sales_quotation_items(item_id);`);

  // projects (without sales_order_id FK - added later)
  await knex.raw(`CREATE TABLE projects (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_projects_project_code ON projects(project_code);
...
CREATE INDEX idx_projects_status ON projects(status);`);
  ...
  // sales_orders
  await knex.raw(`CREATE TABLE sales_orders (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_sales_orders_sales_order_number ON sales_orders(sales_order_number);
...
CREATE INDEX idx_sales_orders_status ON sales_orders(status);`);
  ...
  // Add sales_order_id FK to projects
  await knex.raw('ALTER TABLE projects ADD CONSTRAINT fk_projects_sales_order_id FOREIGN KEY (sales_order_id) REFERENCES sales_orders(id) ON DELETE SET NULL');

  // sales_order_items
  await knex.raw(`CREATE TABLE sales_order_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_sales_order_items_sales_order_id ON sales_order_items(sales_order_id);
...
CREATE INDEX idx_sales_order_items_item_id ON sales_order_items(item_id);`);
  ...
  await knex.raw(`CREATE TABLE project_budget_categories (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_project_budget_categories_project_id ON project_budget_categories(project_id);`);

  // purchase_requests
  await knex.raw(`CREATE TABLE purchase_requests (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_purchase_requests_pr_number ON purchase_requests(pr_number);
...
CREATE INDEX idx_purchase_requests_deadline_date ON purchase_requests(deadline_date);`);
  ...
  await knex.raw(`CREATE TABLE purchase_request_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_purchase_request_items_pr_id ON purchase_request_items(pr_id);
CREATE INDEX idx_purchase_request_items_item_id ON purchase_request_items(item_id);`);
  ...
  await knex.raw(`CREATE INDEX idx_vendors_vendor_code ON vendors(vendor_code);
...
CREATE INDEX idx_vendors_status ON vendors(status);`);
  ...
  await knex.raw(`CREATE INDEX idx_purchase_orders_po_number ON purchase_orders(po_number);
...
CREATE INDEX idx_purchase_orders_status ON purchase_orders(status);`);
  ...
  await knex.raw(`CREATE TABLE purchase_order_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_purchase_order_items_po_id ON purchase_order_items(po_id);
CREATE INDEX idx_purchase_order_items_item_id ON purchase_order_items(item_id);`);

  // goods_received_notes
  await knex.raw(`CREATE TABLE goods_received_notes (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_grn_grn_number ON goods_received_notes(grn_number);
...
CREATE INDEX idx_grn_received_date ON goods_received_notes(received_date);`);
  ...
  // grn_items
  await knex.raw(`CREATE TABLE grn_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_grn_items_grn_id ON grn_items(grn_id);
CREATE INDEX idx_grn_items_po_item_id ON grn_items(po_item_id);`);

  // vendor_bills
  await knex.raw(`CREATE TABLE vendor_bills (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_vendor_bills_bill_number ON vendor_bills(bill_number);
...
CREATE INDEX idx_vendor_bills_invoice_number ON vendor_bills(invoice_number);`);
  ...
  // vendor_bill_items
  await knex.raw(`CREATE TABLE vendor_bill_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_vendor_bill_items_bill_id ON vendor_bill_items(bill_id);
CREATE INDEX idx_vendor_bill_items_po_item_id ON vendor_bill_items(po_item_id);`);

  // vendor_payments
  await knex.raw(`CREATE TABLE vendor_payments (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_vendor_payments_payment_number ON vendor_payments(payment_number);
...
CREATE INDEX idx_vendor_payments_status ON vendor_payments(status);`);
  ...
  await knex.raw(`CREATE TABLE vendor_payment_allocations (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_vendor_payment_allocations_payment_id ON vendor_payment_allocations(payment_id);
CREATE INDEX idx_vendor_payment_allocations_bill_id ON vendor_payment_allocations(bill_id);`);

  // delivery_orders
  await knex.raw(`CREATE TABLE delivery_orders (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_delivery_orders_do_number ON delivery_orders(do_number);
...
CREATE INDEX idx_delivery_orders_delivery_date ON delivery_orders(delivery_date);`);
  ...
  await knex.raw(`CREATE TABLE delivery_order_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_delivery_order_items_do_id ON delivery_order_items(do_id);
CREATE INDEX idx_delivery_order_items_item_id ON delivery_order_items(item_id);`);

  // delivery_order_documents
  await knex.raw(`CREATE TABLE delivery_order_documents (
    id SERIAL PRIMARY KEY,
    do_id INTEGER NOT NULL REFERENCES delivery_orders(id) ON DELETE CASCADE,
  ...
  await knex.raw(`CREATE INDEX idx_delivery_order_documents_do_id ON delivery_order_documents(do_id);`);

  // customer_invoices
  await knex.raw(`CREATE TABLE customer_invoices (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_customer_invoices_invoice_number ON customer_invoices(invoice_number);
...
CREATE INDEX idx_customer_invoices_due_date ON customer_invoices(due_date);`);
  ...
  await knex.raw(`CREATE TABLE customer_invoice_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_customer_invoice_items_invoice_id ON customer_invoice_items(invoice_id);
CREATE INDEX idx_customer_invoice_items_item_id ON customer_invoice_items(item_id);`);

  // customer_payments
  await knex.raw(`CREATE TABLE customer_payments (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_customer_payments_payment_number ON customer_payments(payment_number);
...
CREATE INDEX idx_customer_payments_status ON customer_payments(status);`);
  ...
  await knex.raw(`CREATE TABLE customer_payment_allocations (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_customer_payment_allocations_payment_id ON customer_payment_allocations(payment_id);
CREATE INDEX idx_customer_payment_allocations_invoice_id ON customer_payment_allocations(invoice_id);`);

  // timesheets
  await knex.raw(`CREATE TABLE timesheets (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_timesheets_timesheet_number ON timesheets(timesheet_number);
...
CREATE INDEX idx_timesheets_status ON timesheets(status);`);
  ...
  // timesheet_entries
  await knex.raw(`CREATE TABLE timesheet_entries (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_timesheet_entries_timesheet_id ON timesheet_entries(timesheet_id);`);

  // documents
  await knex.raw(`CREATE TABLE documents (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_documents_entity ON documents(entity_type, entity_id);
...
CREATE INDEX idx_notifications_created_at ON notifications(created_at);`);
  ...
  // audit_logs
  await knex.raw(`CREATE TABLE audit_logs (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_audit_logs_action ON audit_logs(action);`);
  ...
  // system_settings
  await knex.raw(`CREATE TABLE system_settings (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_system_settings_key ON system_settings(key);`);

  // client_purchase_orders
  await knex.raw(`CREATE TABLE client_purchase_orders (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_client_pos_client_po_number ON client_purchase_orders(client_po_number);
...
CREATE INDEX idx_client_pos_status ON client_purchase_orders(status);`);
  ...
  await knex.raw(`CREATE TABLE client_purchase_order_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_client_po_items_client_po_id ON client_purchase_order_items(client_po_id);
...
CREATE INDEX idx_client_po_items_item_id ON client_purchase_order_items(item_id);`);
  ...
  // communication_logs
  await knex.raw(`CREATE TABLE communication_logs (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_communication_logs_entity ON communication_logs(entity_type, entity_id);
...
CREATE INDEX idx_communication_logs_date ON communication_logs(communication_date);`);
  ...
  // rate_cards
  await knex.raw(`CREATE TABLE rate_cards (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_rate_cards_code ON rate_cards(rate_card_code);
...
CREATE INDEX idx_rate_cards_dates ON rate_cards(effective_from, effective_to);`);
  ...
  // vouchers
  await knex.raw(`CREATE TABLE vouchers (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_vouchers_voucher_number ON vouchers(voucher_number);
...
CREATE INDEX idx_vouchers_voucher_date ON vouchers(voucher_date);`);
  ...
  // transmittal_forms
  await knex.raw(`CREATE TABLE transmittal_forms (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_transmittal_forms_transmittal_number ON transmittal_forms(transmittal_number);
...
CREATE INDEX idx_transmittal_forms_date ON transmittal_forms(transmittal_date);`);
  ...
  // item_receipts
  await knex.raw(`CREATE TABLE item_receipts (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_item_receipts_receipt_number ON item_receipts(receipt_number);
...
CREATE INDEX idx_item_receipts_do_required ON item_receipts(do_required);`);
  ...
  // item_receipt_items
  await knex.raw(`CREATE TABLE item_receipt_items (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_item_receipt_items_receipt_id ON item_receipt_items(item_receipt_id);
CREATE INDEX idx_item_receipt_items_po_item_id ON item_receipt_items(po_item_id);`);

  // attendance
  await knex.raw(`CREATE TABLE attendance (
    id SERIAL PRIMARY KEY,
...
CREATE INDEX idx_attendance_type ON attendance(attendance_type);`);
  ...
  // project_schedules
  await knex.raw(`CREATE TABLE project_schedules (
    id SERIAL PRIMARY KEY,
    project_id INTEGER NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  ...
  await knex.raw(`CREATE INDEX idx_project_schedules_project_id ON project_schedules(project_id);
...
CREATE INDEX idx_project_schedules_status ON project_schedules(status);`);
  ...
  // work_in_progress
  await knex.raw(`CREATE TABLE work_in_progress (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_wip_wip_number ON work_in_progress(wip_number);
...
CREATE INDEX idx_wip_progress_date ON work_in_progress(progress_date);`);
  ...
  // wip_material_usage
  await knex.raw(`CREATE TABLE wip_material_usage (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_wip_material_usage_wip_id ON wip_material_usage(wip_id);
CREATE INDEX idx_wip_material_usage_item_id ON wip_material_usage(item_id);`);

  // project_tasks
  await knex.raw(`CREATE TABLE project_tasks (
    id SERIAL PRIMARY KEY,
    project_id INTEGER NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
    task_code VARCHAR(50),
  ...
  await knex.raw(`CREATE INDEX idx_project_tasks_project_id ON project_tasks(project_id);
...
CREATE INDEX idx_project_tasks_assigned_to ON project_tasks(assigned_to);`);
  ...
  // vendor_prepayments
  await knex.raw(`CREATE TABLE vendor_prepayments (
    id SERIAL PRIMARY KEY,
  ...
  await knex.raw(`CREATE INDEX idx_vendor_prepayments_prepayment_number ON vendor_prepayments(prepayment_number);
...
exports.down = async function(knex) {
  await knex.raw('ALTER TABLE projects DROP CONSTRAINT IF EXISTS fk_projects_sales_order_id');
  await knex.raw('ALTER TABLE organizations DROP CONSTRAINT IF EXISTS fk_organizations_created_by');
  await knex.raw('DROP TABLE IF EXISTS vendor_prepayments CASCADE');
  await knex.raw('DROP TABLE IF EXISTS project_tasks CASCADE');
  await knex.raw('DROP TABLE IF EXISTS wip_material_usage CASCADE');
  await knex.raw('DROP TABLE IF EXISTS work_in_progress CASCADE');
  await knex.raw('DROP TABLE IF EXISTS project_schedules CASCADE');
  await knex.raw('DROP TABLE IF EXISTS attendance CASCADE');
  await knex.raw('DROP TABLE IF EXISTS item_receipt_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS item_receipts CASCADE');
  await knex.raw('DROP TABLE IF EXISTS transmittal_forms CASCADE');
  await knex.raw('DROP TABLE IF EXISTS vouchers CASCADE');
  await knex.raw('DROP TABLE IF EXISTS rate_cards CASCADE');
  await knex.raw('DROP TABLE IF EXISTS communication_logs CASCADE');
  await knex.raw('DROP TABLE IF EXISTS client_purchase_order_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS client_purchase_orders CASCADE');
  ...
  await knex.raw('DROP TABLE IF EXISTS timesheet_entries CASCADE');
  await knex.raw('DROP TABLE IF EXISTS timesheets CASCADE');
  await knex.raw('DROP TABLE IF EXISTS customer_payment_allocations CASCADE');
  await knex.raw('DROP TABLE IF EXISTS customer_payments CASCADE');
  await knex.raw('DROP TABLE IF EXISTS customer_invoice_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS customer_invoices CASCADE');
  await knex.raw('DROP TABLE IF EXISTS delivery_order_documents CASCADE');
  await knex.raw('DROP TABLE IF EXISTS delivery_order_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS delivery_orders CASCADE');
  await knex.raw('DROP TABLE IF EXISTS vendor_payment_allocations CASCADE');
  await knex.raw('DROP TABLE IF EXISTS vendor_payments CASCADE');
  await knex.raw('DROP TABLE IF EXISTS vendor_bill_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS vendor_bills CASCADE');
  await knex.raw('DROP TABLE IF EXISTS grn_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS goods_received_notes CASCADE');
  await knex.raw('DROP TABLE IF EXISTS purchase_order_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS purchase_orders CASCADE');
  await knex.raw('DROP TABLE IF EXISTS vendors CASCADE');
  await knex.raw('DROP TABLE IF EXISTS purchase_request_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS purchase_requests CASCADE');
  await knex.raw('DROP TABLE IF EXISTS project_budget_categories CASCADE');
  await knex.raw('DROP TABLE IF EXISTS sales_order_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS sales_orders CASCADE');
  await knex.raw('DROP TABLE IF EXISTS projects CASCADE');
  await knex.raw('DROP TABLE IF EXISTS sales_quotation_items CASCADE');
  await knex.raw('DROP TABLE IF EXISTS sales_quotations CASCADE');
  await knex.raw('DROP TABLE IF EXISTS sales_enquiries CASCADE');
  await knex.raw('DROP TABLE IF EXISTS items CASCADE');
